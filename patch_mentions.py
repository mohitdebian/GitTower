...
# 1. Add state imports and hooks if not present
# The file already has import { useEffect, useState } from 'react';
# Let's change it to import { useEffect, useState, useMemo, useRef } from 'react';
content = content.replace("import { useEffect, useState } from 'react';", "import { useEffect, useState, useMemo, useRef } from 'react';")
...
state_replace = """  const [selectedItem, setSelectedItem] = useState<GitHubIssue | null>(null);
  ...
  const insertMention = (login: string) => {
    ...
    if (match) {
      ...
      setMentionQuery(null);
      // We would ideally set the cursor position after the inserted text here, but React state updates make it tricky.
      setTimeout(() => {
